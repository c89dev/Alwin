...
export async function saveInventoryToDisk(dataToSave: InventoryItem[]) {
    const response = await fetch("http://localhost:3000/api/saveinventory", {
        method: "POST",
        headers: {
...
export async function saveIdCountToDisk(dataToSave: number) {
    const response = await fetch("http://localhost:3000/api/saveidcount", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
...
export async function loadIdCounterFromDisk(): Promise<number> {
    const response = await fetch("http://localhost:3000/api/loadidcount");
    const data = await response.json();
